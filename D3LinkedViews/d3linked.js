...
d3.json("output.json", function(JSONdata) {
	...
	var basic_choropleth = new Datamap({
		...
		// clickable
		done: function(map){ map.svg.selectAll('.datamaps-subunit').on('click', function(country) { draw(country.properties.name); })},
...
function draw(country) {
	document.getElementById("knetter").innerHTML = country;	
	...
	d3.json("bardata.json", function(Bardata) {
		// check if the country name of eacht abbreviation is in the json list
		for (i = 0, n = Bardata['points'].length; i < n; i++) {	
			// if so, replace the name with the abbreviation
			if (Bardata['points'][i]["Country"] == country) {
				wellbeing = Bardata['points'][i]["Well-Being"] * 10;
				footprint = Bardata['points'][i]["Footprint"] * 10;
				life = Bardata['points'][i]["Life Expectancy"];
				data.push(wellbeing, footprint, life);
				console.log(data);
			}
		}
	...
	bar.append("text")
		...
		.attr("y", barHeight / 2)
		.attr("dy", ".35em")
		.text(function(d, i) { if ( i == 0 ) { return "Well Being: " + Math.round(d * 10) / 100; } 
							   if ( i == 1) { return "Footprint: " + Math.round(d * 10) / 100 } 
							   if ( i == 2) { return "Life Expectancy: " + d  }
		});	
